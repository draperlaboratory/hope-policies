...
RHEAL_DEADLK_BASE_TARGET = $(OUTPUT_DIR)/rhealstone_deadlk_base
RHEAL_DEADLK_BASE_SRC    = $(RHEAL_SRC_DIR)/main_deadlock_break.c
RHEAL_DEADLK_BASE_OBJ    =  $(patsubst %.c,%.o,$(RHEAL_DEADLK_BASE_SRC))

.PHONY: clean-rhealstone clean-libfreertos clean-all build-libfreertos
all: $(RHEAL_DEADLK_BASE_TARGET)

# Binary Targets _______________________________________________________________
$(RHEAL_DEADLK_BASE_TARGET): $(ISP_OBJECTS) $(ISP_DEPS) $(ISP_LIBFREERTOS)  $(COMMON_RHEAL_OBJS) $(RHEAL_DEADLK_BASE_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) $(INCLUDES) $(ISP_LIB_INCS) $(ISP_LIB_FLAGS) $(ISP_OBJECTS) \
	$(COMMON_RHEAL_OBJS) $(RHEAL_DEADLK_BASE_OBJ) -o $@

# Object Targets  ______________________________________________________________
$(COMMON_RHEAL_OBJS): %.o: %.c $(COMMON_RHEAL_SRCS)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@

$(RHEAL_DEADLK_BASE_OBJ): $(RHEAL_DEADLK_BASE_SRC)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@
