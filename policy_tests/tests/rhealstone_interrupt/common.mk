...
MACHINE_SW_INTERRUPT_HANDLER=handle_trap_rhealstone
...
ISP_LIBFREERTOS_INT_HANDLER = $(FREERTOS_BUILD_DIR)/libfreertos_int_handler.a
ISP_LIBFREERTOS = $(ISP_LIBFREERTOS_INT_HANDLER)
...
# not using $(RHEAL_SRC_DIR)/test_common.c because taken care of in libfreertos.a
COMMON_RHEAL_SRCS = \
...
RHEAL_INTERRUPT_TARGET   = $(OUTPUT_DIR)/rhealstone_interrupt
RHEAL_INTERRUPT_SRC      = $(RHEAL_SRC_DIR)/main_interrupt_latency.c
RHEAL_INTERRUPT_OBJ      =  $(patsubst %.c,%.o,$(RHEAL_INTERRUPT_SRC))

.PHONY: clean-rhealstone clean-libfreertos clean-all build-libfreertos
all: $(RHEAL_INTERRUPT_TARGET)

# Binary Targets _______________________________________________________________
$(RHEAL_INTERRUPT_TARGET): $(ISP_OBJECTS) $(ISP_LIBFREERTOS_INT_HANDLER) $(ISP_DEPS) $(COMMON_RHEAL_OBJS) $(RHEAL_INTERRUPT_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) $(INCLUDES) $(ISP_LIB_INCS) $(ISP_LIB_FLAGS) $(ISP_OBJECTS) \
	$(COMMON_RHEAL_OBJS) $(RHEAL_INTERRUPT_OBJ) -o $@

# Object Targets  ______________________________________________________________
$(COMMON_RHEAL_OBJS): %.o: %.c $(COMMON_RHEAL_SRCS)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@

$(RHEAL_INTERRUPT_OBJ): $(RHEAL_INTERRUPT_SRC)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@
...
$(ISP_LIBFREERTOS_INT_HANDLER):
	...
	MSI_HANDLER=$(MACHINE_SW_INTERRUPT_HANDLER) \
	FREERTOS_CONFIG_INC=$(abspath $(RHEAL_FREERTOS_CONFIG_DIR)) \
	build/libfreertos_int_handler.a
