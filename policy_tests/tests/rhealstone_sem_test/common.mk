...
RHEAL_SEM_TEST_TARGET    = $(OUTPUT_DIR)/rhealstone_sem_test
RHEAL_SEM_TEST_SRC       = $(RHEAL_SRC_DIR)/main_semaphore_shuffle.c
RHEAL_SEM_TEST_OBJ       =  $(patsubst %.c,%.o,$(RHEAL_SEM_TEST_SRC))

.PHONY: clean-rhealstone clean-libfreertos clean-all build-libfreertos
all: $(RHEAL_SEM_TEST_TARGET)


# Binary Targets _______________________________________________________________
$(RHEAL_SEM_TEST_TARGET): $(ISP_OBJECTS) $(ISP_DEPS) $(ISP_LIBFREERTOS) $(COMMON_RHEAL_OBJS) $(RHEAL_SEM_TEST_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) $(INCLUDES) $(ISP_LIB_INCS) $(ISP_LIB_FLAGS) $(ISP_OBJECTS) \
	$(COMMON_RHEAL_OBJS) $(RHEAL_SEM_TEST_OBJ) -o $@

# Object Targets  ______________________________________________________________
$(COMMON_RHEAL_OBJS): %.o: %.c $(COMMON_RHEAL_SRCS)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@

$(RHEAL_SEM_TEST_OBJ): $(RHEAL_SEM_TEST_SRC)
	$(CC) -DBENCH_DISABLE_PREEMTION -DBENCH_USE_SEMAPHORE $(CFLAGS) $(INCLUDES) $< -c -o $@
