...
RHEAL_SEM_BASE_TARGET    = $(OUTPUT_DIR)/rhealstone_sem_base
RHEAL_SEM_BASE_SRC       = $(RHEAL_SRC_DIR)/main_semaphore_shuffle.c
RHEAL_SEM_BASE_OBJ       =  $(patsubst %.c,%.o,$(RHEAL_SEM_BASE_SRC))

.PHONY: clean-rhealstone clean-libfreertos clean-all build-libfreertos
all: $(RHEAL_SEM_BASE_TARGET)

# Binary Targets _______________________________________________________________
$(RHEAL_SEM_BASE_TARGET): $(ISP_OBJECTS) $(ISP_DEPS) $(ISP_LIBFREERTOS_NO_PREEMPT) $(COMMON_RHEAL_OBJS) $(RHEAL_SEM_BASE_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) $(INCLUDES) $(ISP_LIB_INCS) $(ISP_LIB_FLAGS) $(ISP_OBJECTS) \
	$(COMMON_RHEAL_OBJS) $(RHEAL_SEM_BASE_OBJ) -o $@

# Object Targets  ______________________________________________________________
$(COMMON_RHEAL_OBJS): %.o: %.c $(COMMON_RHEAL_SRCS)
	$(CC) $(CFLAGS) $(INCLUDES) $< -c -o $@

$(RHEAL_SEM_BASE_OBJ): $(RHEAL_SEM_BASE_SRC)
	$(CC) -DBENCH_DISABLE_PREEMTION $(CFLAGS) $(INCLUDES) $< -c -o $@
